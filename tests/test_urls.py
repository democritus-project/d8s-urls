...
def test_url_canonical_form_docs_1():
    # url_canonical_form quotes unsafe characters in the path and query string (matching the behavior of the
    # now-removed werkzeug.urls.url_fix); it does not normalize the scheme, host, port, or fragment.
    ...
    assert url_canonical_form("www.google.com/") == "www.google.com/"
    assert url_canonical_form("www.google.com") == "www.google.com"
    assert url_canonical_form("http://www.evil.com/blah#frag") == "http://www.evil.com/blah#frag"
    assert url_canonical_form("http://www.GOOgle.com/") == "http://www.GOOgle.com/"
    assert url_canonical_form("http://www.google.com.../") == "http://www.google.com.../"
    ...
    assert url_canonical_form("http://www.google.com/q?r?") == "http://www.google.com/q?r%3F"
    assert url_canonical_form("http://www.google.com/q?r?s") == "http://www.google.com/q?r%3Fs"
    assert url_canonical_form("http://evil.com/foo#bar#baz") == "http://evil.com/foo#bar#baz"
    assert url_canonical_form("http://evil.com/foo;") == "http://evil.com/foo%3B"
    assert url_canonical_form("http://evil.com/foo?bar;") == "http://evil.com/foo?bar%3B"
    assert url_canonical_form("http://\x01\x80.com/") == "http://\x01\x80.com/"
    assert url_canonical_form("http://notrailingslash.com") == "http://notrailingslash.com"
    assert url_canonical_form("http://www.gotaport.com:1234/") == "http://www.gotaport.com:1234/"
    assert url_canonical_form("  http://www.google.com/  ") == "http://www.google.com/%20%20"
    assert url_canonical_form("http:// leadingspace.com/") == "http:// leadingspace.com/"
    assert url_canonical_form("http://%20leadingspace.com/") == "http://%20leadingspace.com/"
    assert url_canonical_form("%20leadingspace.com/") == "%20leadingspace.com/"
    ...
    assert (
        url_canonical_form("http://host.com//twoslashes?more//slashes")
...
@pytest.mark.xfail(
    reason="pre-existing, unrelated to tooling migration: live network call to an external URL-ranking service",
    strict=False,
)
...
@pytest.mark.xfail(
    reason="pre-existing, unrelated to tooling migration: live network call to an external screenshot service",
    strict=False,
)
